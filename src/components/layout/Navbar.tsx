...
import { Bell, Globe2, LayoutDashboard, Search, HeartPulse, UserCircle } from "lucide-react";
...
const navLinks = [
    ...
    {
        name: isAuthenticated ? (user?.profile.name.split(' ')[0] || "Account") : "Sign In",
        path: isAuthenticated ? "/account" : "/login",
        icon: isAuthenticated ? <UserCircle className="w-4 h-4" /> : <LayoutDashboard className="w-4 h-4" />
    },
];
    ...
    <nav className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/80 backdrop-blur-2xl">
        <div className="container mx-auto px-4 lg:px-8 h-20 flex items-center justify-between">
            <Link href="/" className="flex items-center gap-2 group">
                ...
                <span className="font-logo text-2xl font-bold tracking-wide mt-1 text-foreground">EmPact</span>
            </Link>
