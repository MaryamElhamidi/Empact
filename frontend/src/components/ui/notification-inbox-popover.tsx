                ...
                <Tabs value={tab} onValueChange={setTab}>
                    <div className="flex items-center justify-between border-b border-border/40 px-3 py-2 bg-muted/20">
                        <TabsList className="bg-transparent gap-2 h-auto p-0 border-none">
                            <TabsTrigger value="all" className="text-xs font-bold h-8 px-3 rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm">All</TabsTrigger>
                            <TabsTrigger value="unread" className="text-xs font-bold h-8 px-3 rounded-lg data-[state=active]:bg-background data-[state=active]:shadow-sm">
                                Unread {unreadCount > 0 && <Badge size="sm" className="ml-1 bg-primary/10 text-primary border-none font-bold">{unreadCount}</Badge>}
                            </TabsTrigger>
                        </TabsList>
                        {unreadCount > 0 && (
                            <button
                    ...
                    <div className="max-h-[400px] overflow-y-auto overflow-x-hidden divide-y divide-border/20">
                        {filtered.length === 0 ? (
                            <div className="px-6 py-12 text-center text-sm text-muted-foreground flex flex-col items-center gap-3">
                                ...
                                <p className="font-medium">No {tab === "unread" ? "unread " : ""}notifications</p>
                            </div>
                        ) : (
                            filtered.map((n) => {
                                const Icon = n.icon;
                                return (
                                    <button
                                        ...
                                        className="flex w-full items-start gap-3 px-4 py-4 text-left hover:bg-muted/30 transition-all group relative"
                                    >
                                        <div className={cn(
                                            "mt-0.5 p-2 rounded-xl transition-all duration-300",
                                            !n.isRead ? "bg-primary/5 text-primary scale-110 shadow-sm" : "bg-muted text-muted-foreground"
                                        )}>
                                            <Icon size={16} />
                                        </div>
                                            ...
                                            <p className="text-[10px] font-bold text-muted-foreground/60 uppercase tracking-widest flex items-center gap-1.5 pt-0.5">
                                                ...
                                                {n.target && (
                                                    <>
                                                        ...
                                                        <span className="text-primary/70">{n.target}</span>
                                                    </>
                                                )}
                                            </p>
                                        </div>
